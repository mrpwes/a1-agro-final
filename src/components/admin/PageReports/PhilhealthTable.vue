...
import { usePhilhealthTableStore } from "stores/admin/reportsPage/philhealthTable";
...
const philhealthTableStore = usePhilhealthTableStore();
...
function exportTable() {
  ...
  const content = [
    philhealthTableStore.columns.map((col) => wrapCsvValue(col.label)),
  ]
    .concat(
      philhealthTableStore.rows.map((row) =>
        philhealthTableStore.columns
          .map((col) =>
            wrapCsvValue(
...
philhealthTableStore.fetchEmployeeReports();
</script>
...
<template>
  <q-table
    ...
    :filter="tableSearch"
    :columns="philhealthTableStore.columns"
    :rows="philhealthTableStore.philhealthAudit"
  ...
  >
    ...
    <template v-slot:top-left>
      ...
      <q-select
        ...
        hide-bottom-space
        input-debounce="0"
        :options="philhealthTableStore.selectedDateOptions"
