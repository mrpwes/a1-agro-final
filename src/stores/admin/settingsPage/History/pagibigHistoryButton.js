...
export const usePagibigHistoryButtonStore = defineStore(
  "pagibigHistoryButtonStore",
  {
    state: () => ({
      viewPrompt: false,
      historyDataRows: [],
    ...
    actions: {
      ...
      async fetchPagibigHistory() {
        try {
          const { data, error } = await supabase
            .from("pagibig_contribution_table_audit")
            .select(
              "*, modified_by:employee!pagibig_contribution_table_audit_emp_id_modified_by_fkey(first_name, last_name, company_employee_id)"
            )
