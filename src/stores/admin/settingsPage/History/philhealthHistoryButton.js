...
export const usePhilhealthHistoryButtonStore = defineStore(
  "philhealthHistoryButtonStore",
  {
    state: () => ({
      viewPrompt: false,
      historyDataRows: [],
    ...
    actions: {
      ...
      async fetchPhilhealthHistory() {
        try {
          const { data, error } = await supabase
            .from("philhealth_contribution_table_audit")
            .select(
              "*, modified_by:employee!philhealth_contribution_table_audit_emp_id_modified_by_fkey(first_name, last_name, company_employee_id)"
            )
