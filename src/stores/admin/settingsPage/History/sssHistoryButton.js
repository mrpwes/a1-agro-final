...
export const useSssHistoryButtonStore = defineStore("sssHistoryButtonStore", {
  state: () => ({
    viewPrompt: false,
    historyDataRows: [],
    historyDataColumns: [
  ...
  actions: {
    ...
    async fetchSssHistory() {
      try {
        const { data, error } = await supabase
          .from("sss_contribution_table_audit")
          .select(
            "*, modified_by:employee!sss_contribution_table_audit_emp_id_modified_by_fkey(first_name, last_name, company_employee_id)"
          )
